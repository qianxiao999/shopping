...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        orderTotalPrice = getOrderPricce(orderItemList);
        Order order = creare(userId,shippingId,orderTotalPrice);
        if (order == null){
    ...
    //计算订单的价格
    private BigDecimal getOrderPricce(List<OrderItem> orderItemList){
    ...
    private OrderVO assembleOrderVO(Order order,List<OrderItem> orderItemList,Integer shippingId){
        ...
        if (orderStatusEnum != null){
            orderVO.setStatusDesc(orderStatusEnum.getDese());
        }
        ...
        if (paymentEnum != null){
            orderVO.setStatusDesc(paymentEnum.getDese());
        }
    ...
    * 扣库存**/
    ...
    /**创建订单
     * */
    private Order creare(Integer userId, Integer shippingId, BigDecimal orderTotalPrice){
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
       ...
       for (Cart cart:cartList){
           ...
           if (product == null){
               return ServerResponse.createServerResponseByError("商品id为"+cart.getProductId()+"的商品不存在");
           }
           if (product.getStatus() != Const.ProductStatusEnum.PRODUCT_ONLINE.getCode()){//商品已经下架
               return ServerResponse.createServerResponseByError("id为"+product.getId()+"商品已经下架");
           }
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        cartOrderItemVO.setTotalPrice(getOrderPricce(orderItemList));
        //step4:返回结果
    ...
    public ServerResponse list(Integer userId,Integer pageNum,Integer pageSize) {
        ...
        List<Order> ordersList = Lists.newArrayList();
        if (userId == null){
            //查询所有
            ordersList = orderMapper.selectAll();
        }else {
            //查询当前用户
            ordersList = orderMapper.findOrderByUserid(userId);
        }
        if (ordersList == null||ordersList.size() == 0){
            return ServerResponse.createServerResponseByError("未查询到订单消息");
        }
        List<OrderVO> orderVOList =  Lists.newArrayList();
        for (Order order:ordersList){
            List<OrderItem> orderItemsList = orderItemMapper.findOrderItemsByOrderno(order.getOrderNo());
            OrderVO orderVO = assembleOrderVO(order, orderItemsList, order.getShippingId());
...
//支付接口
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        payInfo.setPayPlatform(Const.PaymentPlatformEnum.ALLPAY.getCode());
        payInfo.setPlatformStatus(trade_status);
        payInfo.setPlatformNumber(trade_no);
        payInfo.setUserId(order.getUserId());
        int insert = payInfoMapper.insert(payInfo);
        if (insert>0){
            return ServerResponse.createServerResponseBySuccess("支付信息保存成功");
        }
    ...
    public ServerResponse query_order_pay_status(Long orderNo) {
        ...
        if (order.getStatus() == Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.createServerResponseBySuccess("已支付",true);
        }
        return ServerResponse.createServerResponseBySuccess("未支付",false);
    }
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://39.96.44.165:8080/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                return ServerResponse.createServerResponseBySuccess("下单成功" ,map);
